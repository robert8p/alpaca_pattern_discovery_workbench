...
(function(){
  ...
  function injectPhase1View(){
    ...
    jobsView?.insertAdjacentHTML('beforebegin',`
      <section class="view" id="view-full-history">
        <div class="readonly-banner"><b>Phase 1 only:</b> infrastructure is installed, but full historical backfill and clustered discovery are deliberately locked. The true sealed holdout begins 4 August 2026.</div>
        ...
        <div class="split-grid">
          ...
          <article class="panel"><div class="panel-head"><div><h2>Phase 1 infrastructure</h2><p>Point-in-time market state, candidate waves and audit controls</p></div></div><div id="fh-infrastructure-status" class="result-stack">Loading…</div></article>
        </div>
        ...
        <div class="callout warning"><strong>No historical run has been launched.</strong><span>The next permitted execution step is a deliberately scoped one-day historical backfill test after deployment and checks are green.</span></div>
      </section>`);
  }
  ...
  async function refreshFullHistory(){
    ...
    const inv=d.historical_source_coverage||{}, b=d.backfill||{}, j=d.resume_retry_state||{}, cp=d.current_processing_partition||{};
    ...
    $q('#fh-backfill-status').innerHTML=`<div class="health-line"><strong>Feature-backfill readiness</strong><span>${esc(b.status||'Architecture ready · no run launched')}</span></div><div class="health-line"><strong>Feature definition</strong><span>v${esc(d.feature_engine_version||'—')} · ${esc(String(d.feature_definition_hash||'').slice(0,16))}</span></div><div class="health-line"><strong>Current partition</strong><span>${cp.chunk_start?`${day(cp.chunk_start)} → ${day(cp.chunk_end)} · ${esc(cp.status)}`:'None'}</span></div><div class="health-line"><strong>Resume / retry</strong><span>${j.status?`${esc(j.status)} · ${esc(j.phase||'—')} · ${n(j.progress_current)}/${n(j.progress_total)} · attempts ${n(j.attempts)}`:'No active historical job'}</span></div><div class="health-line"><strong>Latest error</strong><span>${esc(d.latest_error||'None')}</span></div>`;
  ...
  function openFullHistoryView(){
    ...
    if(subtitle)subtitle.textContent='Phase 1 infrastructure, research chronology and sealed-holdout protection.';
